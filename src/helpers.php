...
/**
 * Return the application base URL from env or empty string.
 */
...
/**
 * Send an email. Uses mail() when available, otherwise logs to storage/emails.log.
 * Respects MAIL_FROM env var. Returns true on success (mail accepted or logged).
 */
function send_email(string $to, string $subject, string $body, array $headers = []): bool
{
    ...
    if (!$from) {
        // try to derive from APP_URL
        $app = getenv('APP_URL') ?: '';
        $host = parse_url($app, PHP_URL_HOST) ?: 'localhost';
        $from = 'no-reply@' . $host;
    }
    ...
    // Ensure lines are not too long
    $bodyWrapped = wordwrap($body, 70);

    // If MAIL_DRIVER=log or mail() is not available/disabled, write to log
    $mailDriver = getenv('MAIL_DRIVER') ?: '';
    if (strtolower($mailDriver) === 'log' || !function_exists('mail')) {
        // safe logging
        $dir = __DIR__ . '/../storage';
        if (!is_dir($dir)) @mkdir($dir, 0775, true);
        $logfile = $dir . '/emails.log';
        $entry = sprintf("[%s] To: %s\nSubject: %s\nHeaders: %s\n\n%s\n\n----\n", date('c'), $to, $subject, $hdr, $bodyWrapped);
        @file_put_contents($logfile, $entry, FILE_APPEND | LOCK_EX);
        return true;
    }

    // Try to send using mail()
    $ok = @mail($to, $subject, $bodyWrapped, $hdr);
    if ($ok) return true;

    // Fallback: log the outgoing email
    try {
        $dir = __DIR__ . '/../storage';
        if (!is_dir($dir)) @mkdir($dir, 0775, true);
        $logfile = $dir . '/emails.log';
        $entry = sprintf("[%s] (fallback) To: %s\nSubject: %s\nHeaders: %s\n\n%s\n\n----\n", date('c'), $to, $subject, $hdr, $bodyWrapped);
        @file_put_contents($logfile, $entry, FILE_APPEND | LOCK_EX);
        return true;
    } catch (\Throwable $e) {
        return false;
